...
Windows 的截图快捷键通常只会把画面复制到剪贴板或打开截图工具，连续整理截图时仍需逐张操作。AltPrtScnCapture 将这个流程变成一次按键：截图自动以 PNG 保存，并可随时把目录中的图片按文件名顺序合并成 PDF。

程序完全在本地运行，不上传截图，不要求管理员权限，也没有第三方运行时依赖。

## 功能亮点

- **可选全局快捷键**：默认拦截 `PrtScn` 并阻止系统截图界面，也可切换为 `Alt + PrtScn`。
...
- **按需管理员模式**：遇到高权限窗口时，可从托盘中一键请求管理员权限重启。
...
5. 按 `PrtScn` 截取当前活动窗口；如有冲突，可在“截图快捷键”子菜单中改用 `Alt + PrtScn`。
...
| 开始检测 | 注册当前选中的全局截图快捷键 |
| 停止检测 | 注销热键，停止自动截图 |
| 截图快捷键 | 在 `PrtScn` 和 `Alt + PrtScn` 之间切换，当前选项会显示勾选 |
| 设置保存目录 | 选择并记住截图输出目录 |
| 合并为PDF | 将当前目录中的 PNG/JPG/JPEG 按文件名排序合并 |
| 以管理员身份重启 | 通过 UAC 重新启动，用于截取管理员或安全软件窗口 |
| 管理员模式 ✓ | 表示当前实例已经使用管理员权限运行 |
| 退出 | 注销热键并关闭程序 |
...
```mermaid
flowchart LR
    A["托盘菜单：开始检测"] --> B["按 PrtScn 或 Alt + PrtScn"]
    B --> C["定位当前活动窗口"]
    C --> D["保存时间戳 PNG"]
    D --> E["托盘通知"]
    D --> F["可选：按文件名合并 PDF"]
...
- 保存目录设置：`%LOCALAPPDATA%\AltPrtScnCapture\settings.txt`。
- 快捷键设置：`%LOCALAPPDATA%\AltPrtScnCapture\hotkey.txt`。
...
- 普通窗口无需管理员权限；仅在需要截取高权限窗口时使用“以管理员身份重启”。
...
- 启用 `PrtScn` 检测时，程序会消费裸按键以阻止 Windows 截图界面；带 `Alt`、`Win`、`Ctrl` 或 `Shift` 的组合键不会被该模式拦截。
- 若低级键盘钩子无法启动，程序会提示切换到 `Alt + PrtScn`；两种快捷键也可随时从托盘菜单切换。
- 普通权限实例无法接收管理员权限窗口中的裸 `PrtScn` 输入。遇到 360 安全卫士、企业管理软件等高权限窗口时，请选择“以管理员身份重启”。
...
**按 PrtScn 时为什么仍然打开 Windows 截图工具？**

请确认托盘中已经选择“开始检测”，并且“截图快捷键 → PrtScn”处于勾选状态。1.1.1 及以上版本会直接阻止系统截图界面，无需修改 Windows 设置；如仍有冲突，可临时切换为 `Alt + PrtScn`。

**为什么浏览器可以截图，但安全软件或企业软件没有生成 PNG？**

目标程序可能使用管理员权限运行。请选择托盘中的“以管理员身份重启”，通过 UAC 后再次选择“开始检测”。提升后的托盘菜单会显示“管理员模式 ✓”。

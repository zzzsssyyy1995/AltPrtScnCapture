...
- **截图模式**：新安装默认 `Alt + PrtScn`；选择“PrtScn 一键截图（管理员）”会申请 UAC，重启后自动开始检测。
...
- **按需管理员模式**：管理员权限与 PrtScn 模式合并，取消 UAC 时原模式继续运行。
...
5. 按 `Alt + PrtScn` 截图，或选择“截图模式 → PrtScn 一键截图（管理员）”，通过 UAC 后直接按 `PrtScn` 截图。
...
| 开始检测 | 注册当前选中的全局截图快捷键 |
| 停止检测 | 注销热键，停止自动截图 |
| 截图模式 | Alt + PrtScn 截图 / PrtScn 一键截图（管理员），当前选项显示勾选 |
| 设置保存目录 | 选择并记住截图输出目录 |
...
- 选择 PrtScn 模式时按需提升权限；切回 Alt + PrtScn 不会降低进程权限，托盘提示会显示实际权限。
...
- PrtScn 模式需要管理员权限。普通启动始终不自动监听；记住 PrtScn 模式时，点击“开始检测”会申请提升。
...
请确认“截图模式 → PrtScn 一键截图（管理员）”已勾选，且托盘提示为“检测中”。

**为什么浏览器可以截图，但安全软件或企业软件没有生成 PNG？**

请选择“截图模式 → PrtScn 一键截图（管理员）”。通过 UAC 后会自动开始检测。取消 UAC 则保留原模式。
